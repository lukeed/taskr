...
function Fly(options) {
	...
	// properties that could not be overwritten
	...
	function isSealed(prop) {
		return prop in sealedProps
	}

	// inject change object into actual fly instance
	function inject(change) {
		if (
			change.label === utils.diffLabels.changed &&
			isSealed(change.name)
		) {
			// execution should crush on attemp to redefine readonly property
			throw new Error('attemp to rewrite sealed property: ' + change.name)
		}

		// if nothing safely inject change
		self[change.name] = change.value
	}

	// isolated context for plugins
	...
	// get differences
	var changes = utils.objectDiff(self, context)
	// for each change inject change
...
Fly.prototype.unwrap = function (onResolved, onRejected) {
	return this._.files.then(flatten).then(onResolved).catch(onRejected)
}
